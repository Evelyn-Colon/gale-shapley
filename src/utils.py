...
def parse_pref(filepath):
    """Parses the input file for preference lists and returns nested lists of hospital and student preferences."""
    ...
    try:
        with open(filepath, 'r') as file:
            ...
            for i in range(n):
                ...
                h_list_small = h_data.strip().split()
            ...
            for i in range(n):
                ...
                s_list_small = s_data.strip().split()
...
def write_matchings(filepath, matchings, output=None):
    """Writes matchings to an output file either specified by the user or derived from the input filepath."""
    # Help with retrieving file information: https://www.pythonmorsels.com/pathlib-module/
    if output is None:
        path_obj = Path(filepath)
        dir = str(path_obj.parent)
        name = str(path_obj.stem)
        output = f"{dir}/{name}.out"
